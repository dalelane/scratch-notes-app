...
/// Open a note that's already inside the notes folder in its own window, showing
/// the full app UI (sidebar + editor) rather than the lightweight preview view.
#[tauri::command]
fn open_note_in_window(app: AppHandle, state: State<AppState>, note_id: String) -> Result<(), String> {
    use std::collections::hash_map::DefaultHasher;
    use std::hash::{Hash, Hasher};
    ...
    if let Some(window) = app.get_webview_window(&label) {
        let _ = app.emit_to(&label, "select-note", note_id);
        window.set_focus().map_err(|e| e.to_string())?;
        return Ok(());
    }
    ...
    let title = state
        ...
        .get(&note_id)
        .map(|note| format!("{} — Scratch", note.title))
        .unwrap_or_else(|| "Scratch".to_string());

    // Pass the note ID via the window URL (same pattern create_preview_window uses
    // for its `file` param) so the new window selects it deterministically as soon
    // as it loads, rather than racing a post-creation event against webview startup.
    let encoded_id = urlencoding::encode(&note_id);
    let url = format!("index.html?note={}", encoded_id);
    ...
    // Focus the new window so it appears on top of the main window (during cold
    // start the main window may otherwise steal focus after its WebView loads).
    let win = window.clone();
    std::thread::spawn(move || {
        std::thread::sleep(std::time::Duration::from_millis(500));
        let _ = win.set_focus();
